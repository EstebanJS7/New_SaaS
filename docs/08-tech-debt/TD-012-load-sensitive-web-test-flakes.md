...
- [ ] Each named test is deterministic: run the full web suite repeatedly under
      parallel load (for example with concurrent CPU work) and observe no
      failure.
- [ ] If the clinical-workspace race was real, the save path carries a test that
      fails without the fix.
- [ ] The full web suite no longer fails intermittently in CI over a period of
      normal PR traffic.
- [ ] This record closed with the evidence that established the cause.
