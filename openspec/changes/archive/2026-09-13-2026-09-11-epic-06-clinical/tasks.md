...
> **WU5 note (2026-09-13):** implemented on
> `feat/epic-06-clinical-wu5-verification` (base: tracker
> `feat/epic-06-clinical` @ `6230f837`, which already contains WU1–WU4B). Live
> evidence was executed locally against a disposable PostgreSQL 16 cluster
> (Docker unavailable; `initdb`/`pg_ctl` from the system PG16 package were
> used). The suite is green at **24/24**; the WU5 slice is not committed and
> requires a fresh review. WU5 measured **1,208 changed lines** excluding
> `.atl/`/`.codegraph/` (530 live-PG test + 410 new delivery docs + 112 updated
> docs + 156 SDD artifacts), over the ≤800-line review budget; the maintainer
> approved the **WU5 `size:exception`** for the honest, non-minified measure
> (the 2026-09-13 review-correction pass raises the post-correction measure to
> ~1,315 changed lines, still covered by the same exception).

- [x] 5.1 RED then GREEN: extend `apps/api/test/live-pg-isolation.e2e-spec.ts`
      for byte-equivalent cross-tenant 404 and parallel autosaves yielding one
      409; threat matrix cases are N/A per design §8. **Implemented** as the
      "EPIC-06 clinical application-path isolation" block (8 tests; suite now
      24/24 locally): live **encounter** CLOSED immutability + encounter
      no-delete trigger enforcement (the five subdomain no-delete triggers are
      pinned statically by the WU1 schema migration test, not live-executed),
      linked audited amendment with the original unchanged, byte-equivalent
      cross-tenant 404 for foreign Patient anchors / encounter UUIDs / record
      UUIDs, negative cross-tenant amendment DB assertions (no amendment row, no
      audit row, own and foreign originals byte-equal before/after, no other
      foreign encounter created), and a deterministic-barrier parallel-autosave
      race yielding exactly one success + one `409 CONFLICT` with one version
      advance and one audit row. Two one-variable RED proofs (removing
      `assertPatient`; removing the version guard) failed the expected clinical
      tests and were reverted byte-for-byte (`clinical.service.ts` sha256
      restored). No threat-matrix cases (design §8 is N/A).
- [x] 5.2 Run root gates, record migration/routes/tests and open questions in
      the change documentation; do not resolve design §10 scope questions
      without a decision. **Implemented**: `pnpm lint`, `pnpm typecheck`,
      `pnpm test`, `pnpm build` and `pnpm --filter @newsaas/api test:live-pg`
      are green; root `pnpm format-check` fails only on the excluded
      pre-existing `.atl/skill-registry.md`, and all WU5 files are
      Prettier-clean. Migration/routes/tests are recorded in `apply-progress.md`
      and the new `docs/01-roadmap/EPIC-06-Clinical.md`,
      `docs/02-stories/VET-004-clinical-encounter.md`,
      `docs/05-modules/Clinical.md` plus roadmap/modules/CHANGELOG/TD-006
      reconciliation. The WU5 `size:exception` is recorded. Design §10 open
      questions (minimal encounter field set; whether `close` requires a
      non-empty `clientSummary`) remain **open and unresolved**.
