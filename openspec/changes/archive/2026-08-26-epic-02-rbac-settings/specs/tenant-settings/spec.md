...
an additive, reversible migration. Rows SHALL be addressable only within the
authenticated tenant context; direct cross-tenant access by row id or namespace
SHALL behave as nonexistent (404 `NOT_FOUND`).
...
- WHEN an authenticated member of tenant A addresses it by id or namespace
- THEN the response is 404 `NOT_FOUND`, and no data is returned or modified
...
unregistered namespace SHALL fail with `VALIDATION_FAILED`. Writes SHALL persist
the registry's current `schemaVersion` on the row. v1 SHALL register exactly one
...
- THEN it raises `VALIDATION_FAILED` without touching storage
...
Settings writes SHALL additionally require the catalog permission key
`sales.settings.manage` (following the `<domain>.settings.manage` convention),
seeded into the permission catalog and granted to OWNER and ADMIN in the
reference matrix. Requests lacking it SHALL receive 403 `FORBIDDEN`.
